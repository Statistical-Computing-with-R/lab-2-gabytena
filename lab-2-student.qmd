...
[1] 30463(rows)    15(columns)

#https://www.r-bloggers.com/2020/06/determine-the-size-of-a-data-frame/
...
ggplot(data = surveys,
       mapping = aes(x = weight, y =  hindfoot_length) ) +
        geom_point(alpha = .3) + facet_wrap(~species) + 
        labs(x = "Weight of Animals grams",
             y = "",
             title = "Relationships of Animals Weight to their Hindfoot Length by Species")
```
...
ggplot(data = surveys, 
  ...
  labs(x = "Weight in grams",
       y = "",
       title = "Distributions of Weight in Animal Species")
```
...
Because the p-value, 0.00, is less than .05, we reject the null. We have convincing evidence that at least one rodent species has a different population mean.
